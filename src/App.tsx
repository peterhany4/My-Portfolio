    ...
    <div className="min-h-screen flex flex-col justify-between p-6 md:p-12 lg:p-24 max-w-5xl mx-auto">
      <header className="flex justify-between items-center mb-16">
        ...
        <div className="text-sm text-zinc-500 dark:text-zinc-400">
          Portfolio Foundation v1.1.0
        </div>
      </header>

      <main className="space-y-12 my-auto">
        <div className="space-y-6">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-50 dark:bg-indigo-950/50 text-indigo-600 dark:text-indigo-400 text-xs font-medium border border-indigo-200/50 dark:border-indigo-800/50">
          ...
          <div className="flex flex-wrap gap-4 pt-4">
            <a
              ...
              className="inline-flex items-center gap-2 px-4 py-2.5 rounded-lg bg-zinc-900 dark:bg-zinc-100 text-white dark:text-zinc-950 font-medium text-sm hover:opacity-95 transition-opacity"
            >
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 px-4 py-2.5 rounded-lg border border-zinc-200 dark:border-zinc-800 font-medium text-sm hover:bg-zinc-50 dark:hover:bg-zinc-900 transition-colors"
            >
        ...
        {/* Data preview section proving content separation */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-8 border-t border-zinc-200 dark:border-zinc-800">
            ...
            <ul className="space-y-2 text-sm text-zinc-600 dark:text-zinc-400">
              {projectsData.map(p => (
                <li key={p.id} className="flex items-center justify-between">
                  <span>{p.title}</span>
                  <span className="text-xs px-2 py-0.5 rounded bg-indigo-100 dark:bg-indigo-950 text-indigo-700 dark:text-indigo-300">{p.status}</span>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div className="p-6 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-900/50">
            ...
            <div className="flex flex-wrap gap-2">
              {skillsData.map(cat => (
                <span key={cat.category} className="text-xs px-2.5 py-1 rounded-md border border-zinc-200 dark:border-zinc-700 font-medium">
      ...
      <footer className="mt-16 pt-8 border-t border-zinc-200 dark:border-zinc-800 text-xs text-zinc-500 dark:text-zinc-400 flex flex-col sm:flex-row justify-between gap-4">
